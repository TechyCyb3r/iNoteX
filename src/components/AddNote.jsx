...
const AddNote = (props) => {
    ...
    const handleClick = (e) => {
        ...
        const trimmedTitle = note.title.trim() || 'Undefined';
        const trimmedDescription = note.description.trim();
        const trimmedTag = note.tag.trim() || 'default';

        if (!trimmedDescription) return showError('✏️ Description cannot be empty', 'warning');
        if (trimmedDescription.length < 5) return showError('📝 Description must be at least 5 characters', 'error');
        if (trimmedTitle.length < 5) return showError('📌 Title must be at least 5 characters', 'error');
            ...
            <Paper elevation={10} sx={{ mx: "auto", p: 4, my: 2, maxWidth: 600, width: '100%', borderRadius: 4, backgroundColor: 'rgba(176, 224, 230, 0.1)', backdropFilter: 'blur(12px)', boxShadow: '0 10px 25px rgba(0,0,0,0.5)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', }} >
                ...
                <Collapse in={error.show}>
                    <Alert severity={error.severity} action={
                        <IconButton color="inherit" size="small" onClick={() => setError({ ...error, show: false })}> <Close fontSize="inherit" />
                        </IconButton>} sx={{ mb: 2 }} > {error.message}
                    </Alert>
                </Collapse>

                <Box component="form" noValidate autoComplete="off">
                    <TextField fullWidth label="Title" variant="outlined" margin="normal" name="title" value={note.title} onChange={onChange} placeholder="Enter a title" InputLabelProps={{ style: { color: '#ccc' } }} InputProps={{ style: { color: '#fff' }, }} />

                    <TextField fullWidth label="Description" multiline rows={4} variant="outlined" margin="normal" name="description" value={note.description} onChange={onChange} placeholder="Write your note here" required InputLabelProps={{ style: { color: '#ccc' } }} InputProps={{ style: { color: '#fff' }, }} />

                    <TextField fullWidth label="Tag (optional)" variant="outlined" margin="normal" name="tag" value={note.tag} onChange={onChange} placeholder="e.g. Work, Personal" InputLabelProps={{ style: { color: '#ccc' } }} InputProps={{ style: { color: '#fff' }, }} />
